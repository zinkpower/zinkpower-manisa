// ZINKPOWER Manisa — modules_core.jsx V10
...
export function Approvals({data,save,user,t}){
  ...
  return <div>
    ...
    <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8,padding:'6px 10px',background:YL+'20',borderRadius:8}}>
    ...
    {open.length===0?<Card><div style={{color:GR,textAlign:'center',padding:16,fontSize:13}}>{t.no_data}</div></Card>:
      open.map(item=>renderCard(item,false))}

    {approved.length>0&&<div style={{marginTop:14}}>
      <div onClick={()=>setShowApproved(s=>!s)} style={{display:'flex',alignItems:'center',gap:8,padding:'8px 12px',background:GN+'15',borderRadius:8,cursor:'pointer'}}>
        <span style={{fontSize:14}}>{showApproved?'▼':'▶'}</span>
        <b style={{fontSize:13,color:GN}}>✓ {t.approved} ({approved.length})</b>
      </div>
      {showApproved&&<div style={{marginTop:8}}>{approved.map(item=>renderCard(item,true))}</div>}
    </div>}

    {rejected.length>0&&<div style={{marginTop:14}}>
      <div onClick={()=>setShowRejected(s=>!s)} style={{display:'flex',alignItems:'center',gap:8,padding:'8px 12px',background:RD+'15',borderRadius:8,cursor:'pointer'}}>
        <span style={{fontSize:14}}>{showRejected?'▼':'▶'}</span>
        <b style={{fontSize:13,color:RD}}>✕ {t.rejected} ({rejected.length})</b>
      </div>
      {showRejected&&<div style={{marginTop:8}}>{rejected.map(item=>renderCard(item,true))}</div>}
    </div>}
  </div>;
}
...
export function Documents({data,save,user,t}){
  ...
  const [delId,setDelId]=useState(null);
  const today=new Date().toISOString().split('T')[0];
  const ef={title:'',category:'plan',version:'1.0',notes:'',date:today,file:null,fileName:''};
  const [f,setF]=useState(ef);
  const isAdmin=user.role==='admin';
  ...
  function handleFile(e){const file=e.target.files&&e.target.files[0];if(!file)return;const r=new FileReader();r.onload=ev=>setF(prev=>({...prev,file:ev.target.result,fileName:file.name}));r.readAsDataURL(file);e.target.value='';}
  function add(){if(!f.title)return;const u=[...items,{...f,id:Date.now(),by:user.name}];setItems(u);save('documents',u);setShow(false);setF(ef);}
    ...
    {show&&canUpload&&<IForm title={`${t.add} ${t.docs}`} onClose={()=>setShow(false)}>
      ...
      <div style={{marginBottom:10}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          <Btn sm outline onClick={()=>fileRef.current&&fileRef.current.click()}>📎 Datei wählen / Dosya Seç</Btn>
      ...
      <div style={{display:'flex',gap:8}}><Btn outline onClick={()=>setShow(false)}>{t.cancel}</Btn><Btn disabled={!f.title} onClick={add}>{t.save}</Btn></div>
    </IForm>}
    {viewFile&&<IForm title="📎 Dokument" onClose={()=>setViewFile(null)}>
      {viewFile.data&&viewFile.data.startsWith('data:image')
        ...
        :<div style={{textAlign:'center',padding:20}}>
          ...
          <a href={viewFile.data} download={viewFile.name||'dokument'}><Btn>⬇️ Herunterladen / İndir</Btn></a>
        </div>}
    </IForm>}
          ...
          <div style={{flex:1}}>
            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4}}>
              ...
              <div>
                ...
                <div style={{fontSize:11,color:GR}}>{item.category} · v{item.version} · {item.date} · 👤 {item.by}</div>
              </div>
            </div>
            {item.notes&&<div style={{fontSize:12,color:GR,marginTop:3}}>{item.notes}</div>}
            <div style={{marginTop:8,display:'flex',gap:6,flexWrap:'wrap'}}>
              {item.file
                ?<Btn sm outline onClick={()=>setViewFile({data:item.file,name:item.fileName})}>📎 {item.fileName||'Dokument öffnen'}</Btn>
